...
from visualization import (
    plot_event_timeline, plot_interarrival_histogram, plot_poisson_pmf,
    plot_monte_carlo, plot_geo_map, plot_magnitude_distribution, plot_cumulative_events,
)
...
/* Sliders */
[data-testid="stSlider"] .stSlider { accent-color: #f97316; }
</style>
...
# Effective rate after magnitude filter
...
with tab1:
    st.markdown("### Earthquake Event Timeline")
    st.plotly_chart(plot_event_timeline(events, _time), use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("### Cumulative Count")
        st.plotly_chart(plot_cumulative_events(events, _time), use_container_width=True)
    with col2:
        st.markdown("### Magnitude Distribution")
        st.plotly_chart(plot_magnitude_distribution(events), use_container_width=True)

    if events:
        st.markdown("### Event Table")
        df = pd.DataFrame(events, columns=["Time (yr)", "Magnitude"])
        df["Year"] = df["Time (yr)"].apply(lambda x: f"{x:.4f}")
        df["Magnitude"] = df["Magnitude"].apply(lambda x: f"{x:.2f}")
        df.index = df.index + 1
        st.dataframe(df[["Year", "Magnitude"]], use_container_width=True, height=280)


# ═══ TAB 2: Interarrival Analysis ═════════════════════════════════════════════
with tab2:
...
with tab4:
    ...
    with risk_col2:
        ...
        p_major = exceedance_probability(_rate, risk_t, risk_k)
